---
title: "Th Panel Post-COMPASS Plots"
author: "Jolie Phan"
output: 
  html_document:
    toc: yes
...
crList <- purrr::pmap(.l = list(parent_nodes_for_compass_runs_rep,
                      ...
                      .f = function(parent, currentStim) {
                        ...
                        crPath <- here::here(sprintf("out/Th_CompassOutput/%s/%s/COMPASSResult_%s_%s.rds", parent, currentStimForFile, parent, currentStimForFile))
                        readRDS(crPath)
                      }) %>% 
...
status_colors <- c("RSTR" = "#984EA3", "LTBI" = "#4DAF4A")
...
cd4_data2plot <- mergeMatricesForPlotCOMPASSResultStack_tmp(crList[CD4RunNames],
                                                            ...
                                                            variable = "Stim")
if(save_output) {
  saveRDS(cd4_data2plot, here::here("processed_data/Merged_Th_CD4_COMPASS_Data.rds"))
}
...
cd8_data2plot <- mergeMatricesForPlotCOMPASSResultStack_tmp(crList[CD8RunNames],
                                                            ...
                                                            variable = "Stim")
if(save_output) {
  saveRDS(cd8_data2plot, here::here("processed_data/Merged_Th_CD8_COMPASS_Data.rds"))
}
...
cd8_merged_COMPASSResult$data$meta$Stim <- factor(dplyr::recode(sub("^8_", "", cd8_merged_COMPASSResult$data$meta$Stim), "CD8_PP1" = "PP1", "CD8_TB_WCL" = "TB WCL"), levels = c("PP1","TB WCL"))
...
```{r}
th_cd4_bg_corr_boxplots <- pmap(.l = list(CD4RunNames,
                                       c("CD4+", "CD4+"),
                                       list(c(-0.0015, 0.05), c(-0.0015, 0.085)),
                                       c(7, 7),
                                       c(1.3, 1.3),
                                       list(NULL, NULL),
                                       c("PP1", "TB WCL")),
                             .f = function(n, p, y, p_text_size, left_cats_heatmap_padding, dichotomize_by_cytokine, main_title) {
                               ...
                               make_boxplot_for_COMPASS_run(crList[[n]], run_name = n, parentSubset = p, add_legend = T, 
                                                            ...
                                                            group_by_order=c("RSTR", "LTBI"),
                                                            group_by_colors=fill_colors,
                                                            group_by_labels = c("RSTR", "LTBI"),
                                                            ...
                                                            save_test_results=TRUE, output_folder=here::here("processed_data"), adj_pval = FALSE, dataset_name = "Th")
                         })
names(th_cd4_bg_corr_boxplots) <- CD4RunNames
if(save_output) {
  saveRDS(th_cd4_bg_corr_boxplots, here::here("processed_data/Th_CD4_make_boxplot_for_COMPASS_run_list.rds"))
}
```

```{r, fig.width=17, fig.height=7}
grid.draw(th_cd4_bg_corr_boxplots[["CD4_PP1"]]$Plot)
```

```{r, fig.width=17, fig.height=7}
grid.draw(th_cd4_bg_corr_boxplots[["CD4_TB_WCL"]]$Plot)
```

## CD8

```{r}
th_cd8_bg_corr_boxplots <- pmap(.l = list(CD8RunNames,
                                       c("CD8+", "CD8+"),
...
names(th_cd8_bg_corr_boxplots) <- CD8RunNames
if(save_output) {
  saveRDS(th_cd8_bg_corr_boxplots, here::here("processed_data/Th_CD8_make_boxplot_for_COMPASS_run_list.rds"))
}
```

```{r, fig.width=17, fig.height=8}
grid.draw(th_cd8_bg_corr_boxplots[["CD8_PP1"]]$Plot)
```

```{r, fig.width=12, fig.height=8}
grid.draw(th_cd8_bg_corr_boxplots[["CD8_TB_WCL"]]$Plot)
```
...
fs_pfs_df$Status <- factor(fs_pfs_df$Status, levels = c("RSTR", "LTBI"))
```
...
if(save_output) {
  ...
  write.csv(fs_pfs_dat_2save,
            here::here("processed_data/RSTR_Th_FS_PFS.csv"), row.names = F)
}
```
...
```{r}
if(save_output) {
  cairo_pdf(file=here::here("out/post_compass_plots/Th_CD4_COMPASS_Heatmap.pdf"), width=5, height=8,
        onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
  print(draw(cd4_heatmap, gap = unit(0.1, "in"), merge_legends = T, heatmap_legend_side = "bottom", annotation_legend_side = "bottom"))
  dev.off()
  
  cairo_pdf(file=here::here("out/post_compass_plots/Th_CD8_COMPASS_Heatmap.pdf"), width=5, height=8,
        onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica
  print(draw(cd8_heatmap, gap = unit(0.1, "in"), merge_legends = T, heatmap_legend_side = "bottom", annotation_legend_side = "bottom"))
  dev.off()
  
  cairo_pdf(file=here::here("out/post_compass_plots/magnitude_plots/Th_CD4_boxplots.pdf"), width=10, height=6,
            onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
  grid.draw(th_cd4_bg_corr_boxplots[["CD4_PP1"]]$Plot)
  grid.newpage()
  grid.draw(th_cd4_bg_corr_boxplots[["CD4_TB_WCL"]]$Plot)
  ...
  cairo_pdf(file=here::here("out/post_compass_plots/magnitude_plots/Th_CD8_boxplots.pdf"), width=10, height=6,
            onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
  grid.draw(th_cd8_bg_corr_boxplots[["CD8_PP1"]]$Plot)
  grid.newpage()
  grid.draw(th_cd8_bg_corr_boxplots[["CD8_TB_WCL"]]$Plot)
  ...
  cairo_pdf(file=here::here("out/post_compass_plots/fs_plots/th_fs_plots.pdf"), width=4, height=5,
        onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
  print(fs_plots)
  dev.off()
}
