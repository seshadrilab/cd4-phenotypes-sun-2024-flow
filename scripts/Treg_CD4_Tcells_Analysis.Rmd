...
# Grab CD8 gate paths
cd8_cd154_path <- "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD8+/CD154+"
cd8_cd137_path <- "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD8+/CD137+"
cd8_ox40_path <- "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD8+/OX40+"
cd8_ctla4_path <- "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD8+/CTLA4+"
...
gs_pop_add(gs, eval(substitute(flowWorkspace::booleanFilter(v),
                               list(v = as.symbol(paste0("", cd8_cd154_path,
                                                         "&", cd8_cd137_path))))),
           parent = "CD8+", name = "CD8_CD154+_OR_CD137+")
...
gs_pop_add(gs, lapply(gs, gh_pop_get_gate, y = cd8_ox40_path),
           parent = "CD8_CD154+_OR_CD137+", name = "OX40+")
gs_pop_add(gs, lapply(gs, gh_pop_get_gate, y = cd8_ctla4_path),
           parent = "CD8_CD154+_OR_CD137+", name = "CTLA4+")
gs_pop_add(gs, eval(substitute(flowWorkspace::booleanFilter(v),
                               list(v = as.symbol(paste0("", cd8_foxp3_path,
                                                         "&", cd8_cd25_path))))),
           parent = "CD8_CD154+_OR_CD137+", name = "FOXP3+CD25+")
...
ox40_pp1_plot <- make_mag_plots(count_list[[nodes_short[1]]], counts_no_outlier = ox40_counts_no_outlier, current_stim = "PP1", num_comparisons = length(stims), groups_to_compare = c("Pneg", "TST+"),
                                                 paired = FALSE, adjust_p = FALSE, fill_colors = fill_colors, group_by_colname = "Status", subtitle = nodes_short[1],
                                                 y_axis_text = "% CD154+|CD137+ CD4 T Cells", y_axis_size = 15,   ylim = c(75, 100))
...
cd39_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(count_list[[nodes_short[8]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("Pneg", "TST+"),
                                                 ...
                                                 y_axis_text = "% CD154+|CD137+ CD4 T Cells", y_axis_size = 15,   ylim = c(0, 30))
                                })
