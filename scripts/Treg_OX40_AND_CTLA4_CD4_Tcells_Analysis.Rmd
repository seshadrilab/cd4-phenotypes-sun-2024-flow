---
title: "Treg Panel OX40+&CTLA4+ CD4 T cells Analysis"
author: "Jolie Phan"
output: 
  html_document:
    toc: yes
...
First, extract the frequencies of OX40+&CTLA4+ CD4 T cells.

```{r}
ox40_ctla4_node <- "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+"

ox40_ctla4_counts <- pData(gs) %>%
  tibble::rownames_to_column("rowname") %>%
  left_join(gs_pop_get_count_fast(gs, subpopulations = ox40_ctla4_node), by = c("rowname" = "name")) %>%
...
ox40_ctla4_counts$Status <- factor(ox40_ctla4_counts$Status, levels = c("RSTR", "LTBI"))
```

Next, extract the frequencies of all other markers.

```{r}
nodes <- c("/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/FOXP3+CD25+",
           "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/CD154+",
           "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/CD137+",
           "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/OX40+",
           "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/CTLA4+",
           "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/CD25+",
           "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/CD39+CD73+",
           "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_OX40+_AND_CTLA4+/IL10+")
nodes_short <- str_replace(nodes, "\\/Time\\/Cells\\/CD3\\+\\CD14\\-\\CD19\\-\\/Singlets\\/Live\\/CD3\\+\\ Lymphocytes\\/CD4\\+\\/CD4\\_\\OX40\\+\\_AND\\_\\CTLA4\\+\\/", "")
...
ox40_ctla4_plots_status <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(ox40_ctla4_counts, current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"), 
                                                 paired = FALSE, adjust_p = FALSE, fill_colors = fill_colors, group_by_colname = "Status", subtitle = "OX40+CTLA4+",
                                                 y_axis_text = "% CD4 T Cells", y_axis_size = 15,   ylim = c(0, 15))
                                })
names(ox40_ctla4_plots_status) <- stims
ox40_ctla4_plots_status
```

Compare among stims.

```{r, fig.width=6, fig.height=5}
ox40_ctla4_plots_stim <- purrr::pmap(.l = list(status),
                                .f = function(n) {
                                  multi_mag_plot(ox40_ctla4_counts, current_status = n,  groups_to_compare = c("DMSO", "PP1", "TB WCL"), 
                                                 subtitle = "OX40+CTLA4+", fill_colors = fill_colors, y_axis_text = "% CD4 T Cells")
                                })
names(ox40_ctla4_plots_stim) <- status
ox40_ctla4_plots_stim
```
...
il10_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(count_list[[nodes_short[8]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"),
                                                 ...
                                                 y_axis_text = "% CD137+&OX40+ CD4 T Cells", y_axis_size = 15,   ylim = c(0, 0.5))
                                })
