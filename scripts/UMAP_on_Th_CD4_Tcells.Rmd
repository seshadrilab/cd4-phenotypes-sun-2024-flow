---
title: "RSTR Dim Red on Th CD4 T cells"
author: "Jolie Phan"
...
cd4_cd154_cd137_parentGate <- "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4 Positive/CD4+/CD4_CD154+_OR_CD137+"
...
```{r}
cols_4_dimred <- c("CD3", "CD8a", "CD4", "CD154", "CD137", "CTLA4", "OX40", "CD45RA", "CCR7", "CXCR3", "CCR6", "RORyT", "TBET", "IFNg", "TBET")
...
if(rerun_dimred) {
  ...
  if(save_output) {
    saveRDS(cd4_cd154_cd137_w_umap, here::here(sprintf("out/UMAP/%s_Th_CD4_CD154_CD137_UMAP_Unsampled.rds", date)))
    }
} else {
  # Assuming UMAP results are already saved
  print("Loading saved UMAP run")
  cd4_cd154_cd137_w_umap <- readRDS(here::here(sprintf("out/UMAP/%s_Th_CD4_CD154_CD137_UMAP_Unsampled.rds", date)))
}
```
...
stim_labs <- c("PP1", "TB WCL")
names(stim_labs) <- c("PP1", "TB WCL")
...
cairo_pdf(file=here::here("out/UMAP/Th_UMAP_by_Batch.pdf"), width=7, height=5,
...
cairo_pdf(file=here::here("out/UMAP/Th_UMAP_by_PTID.pdf"), width=7, height=8,
          onefile = TRUE, bg = "transparent", family = "Arial")
base_dimred_plot("SAMPLE ID")
dev.off()
```
...
cairo_pdf(file=here::here("out/UMAP/Th_UMAP_by_Single_Markers.pdf"), width=6, height=6,
          onefile = TRUE, bg = "transparent", family = "Arial")
for(cg in cd4_gates_for_dimred) {
  print(base_dimred_plot(cg, colorScheme = boolColorScheme) +
          labs(title = sprintf("CD154+|CD137+ CD4 T cells UMAP\nColored by %s", sub(".*\\/([^(\\/)]+)", "\\1", cg))))
