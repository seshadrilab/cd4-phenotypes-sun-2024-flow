...
# RORyT+TBET-
roryt_plots <- purrr::pmap(.l = list(stims, list(c(0, 25), c(0,25), c(0, 8))),
                                .f = function(n, y) {
                                  make_mag_plots(count_list[[nodes_short[8]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"),
                                                 ...
                                                 y_axis_text = "% CD154+|CD137+ CD4 T Cells", y_axis_size = 15,   ylim = y)
                                })
...
```{r, fig.width=4, fig.height=5}
ifng_plots <- purrr::pmap(.l = list(stims, list(c(0,25), c(0,25), c(0,40))),
                                .f = function(n, y) {
...
```{r, fig.width=4, fig.height=5}
il17a_plots <- purrr::pmap(.l = list(stims, list(c(0,5.5), c(0,5.5), c(0,5))),
                                .f = function(n, y) {
...
il17a_counts_no_outlier <- count_list$`IL17a+` %>%
  dplyr::filter(Stim == "PP1") %>%
  dplyr::filter(`SAMPLE ID` != "RS102301")

fmla <- formula(paste("Freq ~ ", "Status"))
test <- wilcox.test(fmla, data = il17a_counts_no_outlier, paired = FALSE)
...
cairo_pdf(file=here::here("out/Th_CD4_Tcells_Analysis/Th_CD4_CXCR3_CCR6_DP_plots.pdf"), width=4, height=5,
          onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
print(cxcr3_ccr6_dp_plots[["DMSO"]])
print(cxcr3_ccr6_dp_plots[["PP1"]])
print(cxcr3_ccr6_dp_plots[["TB WCL"]])
...
cairo_pdf(file=here::here("out/Th_CD4_Tcells_Analysis/Th_CD4_CXCR3_pos_plots.pdf"), width=4, height=5,
          onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
print(cxcr3_plots[["DMSO"]])
print(cxcr3_plots[["PP1"]])
print(cxcr3_plots[["TB WCL"]])
...
cairo_pdf(file=here::here("out/Th_CD4_Tcells_Analysis/Th_CD4_CCR6_pos_plots.pdf"), width=4, height=5,
          onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
print(ccr6_plots[["DMSO"]])
print(ccr6_plots[["PP1"]])
print(ccr6_plots[["TB WCL"]])
