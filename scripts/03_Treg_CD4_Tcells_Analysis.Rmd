...
if(!dir.exists(here::here("out/Treg_CD4_Tcells_Analysis"))) {
  cat(sprintf("Creating folder %s\n", here::here("out/Treg_CD4_Tcells_Analysis")))
  dir.create(here::here("out/Treg_CD4_Tcells_Analysis"), recursive = T)
}
```
...
gs_pop_add(gs, eval(substitute(flowWorkspace::booleanFilter(v),
                               list(v = as.symbol(paste0("", cd4_cd39_path,
                                                         "&", cd4_cd73_path))))),
           parent = "CD4_CD154+_OR_CD137+/FOXP3+CD25+", name = "CD39+CD73+")
...
Extract the frequencies of CD39+CD73+ Tregs

```{r}
cd39_cd73_treg_nodes <- c("/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_CD154+_OR_CD137+/FOXP3+CD25+/CD39+CD73+",
                          "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_CD154+_OR_CD137+/FOXP3+CD25+/CD39+CD73-",
                          "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_CD154+_OR_CD137+/FOXP3+CD25+/CD39-CD73+",
                          "/Time/Cells/CD3+CD14-CD19-/Singlets/Live/CD3+ Lymphocytes/CD4+/CD4_CD154+_OR_CD137+/FOXP3+CD25+/CD39-CD73-")

cd39_cd73_treg_nodes_short <- str_replace(cd39_cd73_treg_nodes, "\\/Time\\/Cells\\/CD3\\+\\CD14\\-\\CD19\\-\\/Singlets\\/Live\\/CD3\\+\\ Lymphocytes\\/CD4\\+\\/CD4\\_\\CD154\\+\\_\\OR\\_\\CD137\\+\\/FOXP3\\+\\CD25\\+\\/", "")

cd39_cd73_treg_count_list <- list()

for(pop in cd39_cd73_treg_nodes) {
  ...
  cd39_cd73_treg_count_list[[pop]] <- current_counts
}

names(cd39_cd73_treg_count_list) <- cd39_cd73_treg_nodes_short
```
...
cd154_cd137_plots_status <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(cd154_cd137_counts, current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"), 
                                                 ...
                                                 y_axis_text = "% CD4 T Cells", y_axis_size = 15,   ylim = c(0, 30))
                                })
...
```{r, fig.width=4, fig.height=5}
# CD39+CD73+
cd39_cd73_dp_treg_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(cd39_cd73_treg_count_list[[cd39_cd73_treg_nodes_short[1]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"), 
                                                 paired = FALSE, adjust_p = FALSE, fill_colors = fill_colors, group_by_colname = "Status", subtitle = cd39_cd73_treg_nodes_short[1],
                                                 y_axis_text = "% CD154+|CD137+ Tregs", y_axis_size = 15,   ylim = c(0, 15))
                                })
names(cd39_cd73_dp_treg_plots) <- stims
cd39_cd73_dp_treg_plots

# CD39+CD73-
cd39_treg_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(cd39_cd73_treg_count_list[[cd39_cd73_treg_nodes_short[2]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"), 
                                                 paired = FALSE, adjust_p = FALSE, fill_colors = fill_colors, group_by_colname = "Status", subtitle = cd39_cd73_treg_nodes_short[2],
                                                 y_axis_text = "% CD154+|CD137+ Tregs", y_axis_size = 15,   ylim = c(0, 100))
                                })
names(cd39_treg_plots) <- stims
cd39_treg_plots

# CD39-CD73+
cd73_treg_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(cd39_cd73_treg_count_list[[cd39_cd73_treg_nodes_short[3]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"), 
                                                 paired = FALSE, adjust_p = FALSE, fill_colors = fill_colors, group_by_colname = "Status", subtitle = cd39_cd73_treg_nodes_short[3],
                                                 y_axis_text = "% CD154+|CD137+ Tregs", y_axis_size = 15,   ylim = c(0, 100))
                                })
names(cd73_treg_plots) <- stims
cd73_treg_plots

# CD39-CD73-
cd39_cd73_dn_treg_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(cd39_cd73_treg_count_list[[cd39_cd73_treg_nodes_short[4]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"), 
                                                 paired = FALSE, adjust_p = FALSE, fill_colors = fill_colors, group_by_colname = "Status", subtitle = cd39_cd73_treg_nodes_short[4],
                                                 y_axis_text = "% CD154+|CD137+ Tregs", y_axis_size = 15,   ylim = c(0, 100))
                                })
names(cd39_cd73_dn_treg_plots) <- stims
cd39_cd73_dn_treg_plots
```
...
foxp3_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(count_list[[nodes_short[4]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"),
                                                 ...
                                                 y_axis_text = "% CD154+|CD137+ CD4 T Cells", y_axis_size = 15,   ylim = c(0, 4))
                                })
...
cd39_cd73_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(count_list[[nodes_short[7]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"),
                                                 ...
                                                 y_axis_text = "% CD154+|CD137+ CD4 T Cells", y_axis_size = 15,   ylim = c(0, 4.5))
                                })
...
ccr7_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
                                  make_mag_plots(count_list[[nodes_short[10]]], current_stim = n, num_comparisons = length(stims), groups_to_compare = c("RSTR", "LTBI"),
                                                 ...
                                                 y_axis_text = "% CD154+|CD137+ CD4 T Cells", y_axis_size = 15,   ylim = c(60, 100))
                                })
...
```{r, fig.width=4, fig.height=5}
foxp3n_cd25p_plots <- purrr::pmap(.l = list(stims),
                                .f = function(n) {
...
names(foxp3n_cd25p_plots) <- stims
foxp3n_cd25p_plots
```
...
```{r}
cairo_pdf(file=here::here("out/Treg_CD4_Tcells_Analysis/Treg_CD4_CD25_plots.pdf"), width=4, height=5,
...
cairo_pdf(file=here::here("out/Treg_CD4_Tcells_Analysis/Treg_CD4_FoxP3_CD25_plots.pdf"), width=4, height=5,
          onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
...
cairo_pdf(file=here::here("out/Treg_CD4_Tcells_Analysis/Treg_CD4_IL10_plots.pdf"), width=4, height=5,
          onefile = TRUE, bg = "transparent", family = "Arial") # default unit is inches, default font is Helvetica.
